 ...
 * @description           将输入的任意对象转换成 Date，如果装换失败将返回当前时间
 */
function getDateObject(datetime) {
  if (datetime instanceof Date) {
    return datetime;
  }
  if (typeof datetime === 'string' && datetime.includes('T') && datetime.includes('.')) { // 兼容IE IOS
 ...
 * @description 预设语言 `zh-cn` 或 `en`；Object: 自定义 locate 对象
 * @description 修改本地化语言
 */
function locate(arg) {
  let newLocale, prop;
  ...
  for (prop in newLocale) {
    if (Object.prototype.hasOwnProperty.call(newLocale, prop) && typeof newLocale[prop] === 'string') {
