<template>
  <card ref="Card">
    <div slot="header">
      ...
      <el-button class="el-icon-download float-right" @click="download">导出</el-button>
    </div>
    <expand-table :data="formData">
      ...
      <el-table-column label="操作" fixed="right" align="center" width="150">
        <template slot-scope="scope">
          ...
          <el-button :disabled="noReceipt(scope.row)" @click.stop="receipt(scope.row)">回执</el-button>
        </template>
      </el-table-column>
    </expand-table>
...
<script>
  import {pageTransferApi, exportTruncateApi} from '@/api/member'
  ...
  export default {
    name: 'Page2',
    components: {Detail, Receipt},
    data() {
      return {
        formData: [],
        state: 2, // 状态 不通过0 审核中1 通过2
        searchName: ''
      }
    },
    mounted() {
    ...
    methods: {
      ...
      download() {
        this.$startLoading()
        exportTruncateApi().then(result => {
          this.$stopLoading()
          const blob = new Blob([result]);
          this.$fileSaver.saveAs(blob, '已通过的组织关系转.xlsx');
        })
          .catch(err => {
            this.$stopLoading()
