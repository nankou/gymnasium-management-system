<template>
  <card ref="Card">
    ...
    <el-table :data="formData">
      ...
      <el-table-column label="文章设置" align="center" width="100">
        <template slot-scope="scope">
          <el-button icon="el-icon-setting" @click.stop="setArticle(scope.row)"/>
        </template>
      </el-table-column>
...
<script>
  import {pageArticleManageApi, delArticleManageApi} from '@/api/study/article';
  ...
  export default {
    ...
    methods: {
      ...
      getData() {
        ...
        pageArticleManageApi(param).then(result => {
          let response = result.resultParam.articleManageDtoPage;
