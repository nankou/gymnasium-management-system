<template>
  <card ref="Card">
    ...
    <el-table :data="formData">
      ...
      <el-table-column prop="learnDuration" label="学习时长(h)">
        <template slot-scope="scope">
          {{$getHour(scope.row.learnDuration)}}
        </template>
      </el-table-column>
...
<script>
  import {pageUserLearnRateApi} from '@/api/study/video';
    ...
    methods: {
      ...
      getData() {
        ...
        pageUserLearnRateApi(param).then(result => {
          let response = result.resultParam.userVideoManageDtoPage;
