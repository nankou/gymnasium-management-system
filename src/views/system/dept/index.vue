  ...
  <card ref="Card">
    ...
    <el-table
      ...
      <el-table-column prop="dutyMan" label="负责人">
        <template slot-scope="scope">
          <span @dblclick="edit(scope.row)">{{scope.row.dutyMan | formatObj('nickName')}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="scoreUser" label="评分人">
        <template slot-scope="scope">
          <span @dblclick="edit(scope.row)">{{scope.row.scoreUser | formatObj('nickName')}}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" fixed="right" align="center" width="210">
        <template slot-scope="scope">
          <el-button type="success" icon="el-icon-search" @click.stop="view(scope.row)" v-permission="'pageUser'"></el-button>
    ...
    <detail ref="Detail" :deptId="deptId"/>
  </card>
</template>

<script>
  import {treeDeptApi, deleteDeptApi} from '@/api/system/dept'
  ...
  export default {
    ...
    methods: {
      ...
      searchData() {
        this.$refs.Card.start();
        treeDeptApi({deptName: this.searchName}).then(result => {
          this.$refs.Card.stop();
          this.formData = result.resultParam.deptTree;
        }).catch(() => {
          this.$refs.Card.stop();
